...
import { ArticleItem, getArticleItem, updateArticleItem } from '@/api/posts/articles';
...
const fetchData = async () => {
  try {
    setLoading(true);
    const { data } = await getArticleItem(Number(route.params.id))
    form.value = data
  } catch (e: any) {
...
const handleSubmit = async ({ values, errors }: any) => {
  ...
  try {
    setSubmitting(true);
    await updateArticleItem(Number(route.params.id), values)
