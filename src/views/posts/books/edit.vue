...
import { BookItem, getBookItem, updateBookItem } from '@/api/posts/books';
...
const form = ref<BookItem>({});

const fetchData = async () => {
  try {
    setLoading(true);
    const { data } = await getBookItem(Number(route.params.id))
    form.value = data
  } catch (e: any) {
...
const handleSubmit = async ({ values, errors }: any) => {
  ...
  try {
    setSubmitting(true);
    await updateBookItem(Number(route.params.id), values)
