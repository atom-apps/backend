...
import { ChapterItem, getChapterItem, updateChapterItem } from '@/api/posts/chapters';
...
const form = ref<ChapterItem>({});

const fetchData = async () => {
  try {
    setLoading(true);
    const { data } = await getChapterItem(Number(route.params.id))
    form.value = data
  } catch (e: any) {
...
const handleSubmit = async ({ values, errors }: any) => {
  ...
  try {
    setSubmitting(true);
    await updateChapterItem(Number(route.params.id), values)
