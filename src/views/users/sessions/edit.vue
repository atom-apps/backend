<template>
  <div>
    <PageHeader title="用户管理" subtitle="在线编辑" :back="true"/>
...
import { SessionItem, getSessionItem, updateSessionItem } from '@/api/users/sessions';
...
const fetchData = async () => {
  try {
    setLoading(true);
    const { data } = await getSessionItem(Number(route.params.id))
    form.value = data
  } catch (e: any) {
