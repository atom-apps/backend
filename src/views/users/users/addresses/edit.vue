...
import { UserAddressItem, getUserAddressItem, updateUserAddressItem } from '@/api/users/user_addresses';
...
const form = ref<UserAddressItem>({});

const fetchData = async () => {
  try {
    setLoading(true);
    const { data } = await getUserAddressItem(Number(route.params.user_id), Number(route.params.id))
    form.value = data
  } catch (e: any) {
...
const handleSubmit = async ({ values, errors }: any) => {
  ...
  try {
    setSubmitting(true);
    await updateUserAddressItem(Number(route.params.user_id), Number(route.params.id), values)
