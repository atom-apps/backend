  ...
  <div>
    <PageHeader subtitle="用户管理" :back="true" :loading="loading">
      <a-button type="primary"
        @click="$router.push({ name: 'UserAddressEdit', params: { id: route.params.id } })">编辑</a-button>
...
import { UserAddressItem, deleteUserAddressItem, getUserAddressItem, tableUserAddressLabels } from '@/api/users/user_addresses';
...
const userInfo = ref<UserAddressItem>({});
const labels = tableUserAddressLabels();

const fetchData = async () => {
  try {
    setLoading(true);
    const { data: profile } = await getUserAddressItem(Number(route.params.user_id), Number(route.params.id))
...
const handleConfirmDelete = async () => {
  try {
    setDeleting(true);
    await deleteUserAddressItem(Number(route.params.user_id), Number(route.params.id))
