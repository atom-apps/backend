<template>
  <div>
    <PageHeader title="用户管理" subtitle="租户列表">
...
const { columns, hidden } = tableTenantColumns();
...
const renderData = ref<TenantItem[]>([]);
...
const fetchData = async (pg: Pagination = basePagination) => {
  ...
  try {
    ...
    } as unknown) as TenantListQuery;
    console.log(params);

    const { data } = await queryTenantList(params);
