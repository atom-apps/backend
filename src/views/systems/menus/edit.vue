...
import { MenuItem, getMenuItem, updateMenuItem } from '@/api/systems/menus';
...
const fetchData = async () => {
  try {
    setLoading(true);
    const { data } = await getMenuItem(Number(route.params.id))
    form.value = data
  } catch (e: any) {
...
const handleSubmit = async ({ values, errors }: any) => {
  ...
  try {
    setSubmitting(true);
    await updateMenuItem(Number(route.params.id), values)
