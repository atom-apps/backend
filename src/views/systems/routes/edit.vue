...
import { RouteItem, getRouteItem, updateRouteItem } from '@/api/systems/routes';
...
const fetchData = async () => {
  try {
    setLoading(true);
    const { data } = await getRouteItem(Number(route.params.id))
    form.value = data
  } catch (e: any) {
...
const handleSubmit = async ({ values, errors }: any) => {
  ...
  try {
    setSubmitting(true);
    await updateRouteItem(Number(route.params.id), values)
