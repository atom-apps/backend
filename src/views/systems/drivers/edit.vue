...
import { DriverItem, getDriverItem, updateDriverItem } from '@/api/storages/drivers';
...
const fetchData = async () => {
  try {
    setLoading(true);
    const { data } = await getDriverItem(Number(route.params.id))
    form.value = data
  } catch (e: any) {
...
const handleSubmit = async ({ values, errors }: any) => {
  ...
  try {
    setSubmitting(true);
    await updateDriverItem(Number(route.params.id), values)
