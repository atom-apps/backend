...
import { LocationItem, getLocationItem, updateLocationItem } from '@/api/systems/locations';
...
const form = ref<LocationItem>({});

const fetchData = async () => {
  try {
    setLoading(true);
    const { data } = await getLocationItem(Number(route.params.id))
    form.value = data
  } catch (e: any) {
...
const handleSubmit = async ({ values, errors }: any) => {
  ...
  try {
    setSubmitting(true);
    await updateLocationItem(Number(route.params.id), values)
